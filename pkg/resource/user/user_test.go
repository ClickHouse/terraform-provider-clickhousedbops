	...
	tests := []resource.TestCase{
		{
			...
			Steps: []resource.TestStep{
				...
				`, sha256, sha256),
					PlanOnly:    true,
					ExpectError: regexp.MustCompile(`Invalid Attribute Combination|one (and only one) of`),
				},
				// test ensure you can't use password_sha256_hash_wo without specifying password_sha256_hash_wo_version too
				{
					Config: fmt.Sprintf(`
					resource "clickhousedbops_user" "test" {
						name                    = "testuser"
				...
				`, sha256, sha256),
					PlanOnly:    true,
					ExpectError: regexp.MustCompile("Invalid Attribute Combination|also requires"),
				},
			},
		},
	}
